...
else
{
	...
	if (!isNull _remorqueur && {
		...
		alive _remorqueur && isNull (_remorqueur getVariable "R3F_LOG_remorque") && (vectorMagnitude velocity _remorqueur < 6) && !(_remorqueur getVariable "R3F_LOG_disabled") && !!(locked _objet in [2,3])
	}) then
	{
